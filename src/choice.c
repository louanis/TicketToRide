...
int chose_move(t_game_info * game_info, MoveData * playMove){ 
    ...
    // Update the biggest route size for each color
    maj_biggest_per_col(game_info);
    // Find the color with the biggest possible route to build
    int col = biggest_col_majoration(game_info);
    // Choose the biggest route of that color
    uint32 road = choose_biggest_color(game_info,game_info->maxSizeCol[col],col);
        
    // If the player does not have a replay
    if(game_info->playerReplay == 0){  

        // Try to build the biggest possible route of length 6
        ...
        // If few wagons left, try to build a route of length 2 or 1
        ...
        // Try to build the biggest route of the chosen color
    ...
    // Reset card picking preferences
    for(int i = 0;i<9;i++) game_info->cardToPick[i] = 0;
    
    // Verify for each card from best to worst to pick if it is available
    for(int i = 1;i<9;i++) {
        ...
        for(int i = 0;i<5;i++) {
            ...
            // Check if this card is desirable
            for(int i = 1;i<9;i++) {
        ...
        // If a good card is found, pick it
        if(we_pick > 0) {
            playMove->action = 3; 
    ...
    // If no route to build or card to pick, draw a random card
    playMove->action = 2;
    return 2; 
...
int chose_obj(t_game_info * game_info, MoveData * myMove, MoveResult * mresult){
    // Find the objective with the highest score
    int max = 0;
    for(int i = 0;i<3;i++){
        if(mresult->objectives[i].score > mresult->objectives[max].score){
            max = i;
        } 
        // Mark all objectives as chosen by default
        myMove->chooseObjectives[i] = 1;
    }
    // Unselect the objective with the highest score dont keep it
...
void build_route(t_game_info * game_info, MoveData * playMove, uint32 cit1, uint32 cit2){
    ...
    // Calculate the number of locomotives needed for the route
    ...
    // Update board and player state after building the route
