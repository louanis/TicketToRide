...
void dijktra(t_matrix_track * matrix, t_dijktra_output * ret, int cit1, int cit2, int (*track_value_dijktra)(const void *, int)){
    int * track_value_arr = (int*) malloc(matrix->size * sizeof(int));
    int * is_in_path = (int*) malloc(matrix->size*sizeof(int)); //0 no 1 yes
    t_track * track_from = (int*) malloc(matrix->size*sizeof(t_track));
